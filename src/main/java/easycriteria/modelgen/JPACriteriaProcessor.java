...
import easycriteria.modelgen.AnnotationProcessorUtils.TypeCategory;
...
public class JPACriteriaProcessor extends AbstractProcessor
{
    ...
    protected void processClass(TypeElement el)
    {
        ...
        {        	
            ...
            try
            {
                ...
                w.append(CODE_INDENT).append("public " + CLASS_NAME_PREFIX + classSimpleName + CLASS_NAME_SUFFIX + "(String attribute, EntityPathNode parent) {");
                w.append("\n");
		    ...
		    if (!isTransient)
		    {
		            ...
		            (member.getKind() == ElementKind.METHOD && AnnotationProcessorUtils.isJavaBeanGetter((ExecutableElement) member)))
		        {
		            ...
		            if (AnnotationProcessorUtils.isFieldJPAAEntity(member)) {
		            	...
		            	//TO-DO handle OBJECT_ATTRIBUTE as well. Or do we really need ObjectAttribute at all???
	...
	private void writePropertyAttribute(String classSimpleName, Map<String, TypeMirror> genericLookups, Writer w,
		...
		if (cat == TypeCategory.ATTRIBUTE || cat == TypeCategory.DATE_ATTRIBUTE || cat == TypeCategory.NUMBER_ATTRIBUTE || cat == TypeCategory.STRING_ATTRIBUTE)
		{
